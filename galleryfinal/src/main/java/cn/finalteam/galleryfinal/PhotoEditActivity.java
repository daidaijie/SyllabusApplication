...
public class PhotoEditActivity extends CropImageActivity implements AdapterView.OnItemClickListener, View.OnClickListener {
    ...
    protected void takeResult(PhotoInfo info) {
        if (GalleryFinal.getFunctionConfig()!=null&&!GalleryFinal.getFunctionConfig().isMutiSelect()) {
            mPhotoList.clear();
        ...
        if (GalleryFinal.getFunctionConfig()!=null)
        mPhotoTempMap.put(info.getPhotoId(), new PhotoTempModel(info.getPhotoPath()));
        if (GalleryFinal.getFunctionConfig()!=null&&!GalleryFinal.getFunctionConfig().isEditPhoto() && mTakePhotoAction) {
            resultAction();
        } else {
            if (GalleryFinal.getFunctionConfig()!=null&&GalleryFinal.getFunctionConfig().isEnablePreview()) {
                mIvPreView.setVisibility(View.VISIBLE);
            }
            if (mPhotoEditListAdapter!=null)
            mPhotoEditListAdapter.notifyDataSetChanged();
    ...
    public   boolean isValidate(Collection<?> list) {
        return list != null && list.size() > 0;
    }

    public   boolean isValidate(Map<?, ?> map) {
